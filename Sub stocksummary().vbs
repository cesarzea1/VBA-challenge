...
For Each ws In Worksheets
    ...
    Dim colrangepricechange As Range
    ...
    For summaryrow = 2 To lastrowsummarytable1
        ...
        'conditional to set ticker values for greatest percent increase in summary table 2
        If ws.Range("q2").Value = ws.Cells(summaryrow, "K").Value Then
        ...
        'conditional to set ticker values for greatest percent decrease in summary table 2
        If ws.Range("q3").Value = ws.Cells(summaryrow, "K").Value Then
    
            'set value for ticker 2 in summary table 2
            tickersummary2 = ws.Cells(summaryrow, "I").Value

            'write ticker in summary table 2 for
            ws.Cells(3, "P").Value = tickersummary2

        End If

        'conditional to set ticker values for greatest volume in summary table 2
        If ws.Range("q4").Value = ws.Cells(summaryrow, "L").Value Then
    
            'set value for ticker 2 in summary table 2
            tickersummary2 = ws.Cells(summaryrow, "I").Value

            'write ticker in summary table 2 for
            ws.Cells(4, "P").Value = tickersummary2

        End If
    Next summaryrow

    'Set the range for the column in the summary table 1 with price change
    Set colrangepricechange = ws.Range("j2:j" & lastrowsummarytable1)
    
    'loop to evaluate values in summary table 2 to set colour coding formats
    For Each cell In colrangepricechange
    
        'conditional to set colour coding to price change in summary table 1
        If cell.Value < 0 Then
        cell.Interior.ColorIndex = 3 'red colour index
        ElseIf cell.Value > 0 Then
        cell.Interior.ColorIndex = 4 'green colour index
        
        End If
        
    Next cell

    'set the format of the percent change columns as percentage in summary table 1
    colrangepercent.NumberFormat = "0.00%"
